package CitireConsola;

import java.util.Scanner;

public class CitireConsolaNotiuniTeoretice1 {

    public static void main(String[] args) {

        Scanner cititor = new Scanner(System.in);
        Scanner cititorText = new Scanner(System.in);

        System.out.println("Introduceti textul dorit");
        String textIntrodus = cititorText.nextLine();
        System.out.println("Am introdus textul " + textIntrodus);

        System.out.println("Introduceti numarul dorit:");
        int numarIntrodus = cititor.nextInt();
        System.out.println("Ai introdus numarul " + numarIntrodus);

        System.out.println("Introduceti textul dorit");
        String textIntrodus2 = cititorText.nextLine();
        System.out.println("Am introdus textul " + textIntrodus2);

        System.out.println("Introduceti un numar double: ");
        double doubleIntrodus = cititor.nextDouble();
        System.out.println("Ai introdus numarul double: " + doubleIntrodus);

        System.out.println("Introduceti un numar float: ");
        float floatIntrodus = cititor.nextFloat();
        System.out.println("Ai introdus numarul float: " + floatIntrodus);

        System.out.println("Introduceti un numar long: ");
        long longIntrodus = cititor.nextLong();
        System.out.println("Ai introdus numarul long: " + longIntrodus);

        System.out.println("Introduceti o valoare boolean: ");
        boolean booleanIntrodus = cititor.hasNextBoolean();
        System.out.println("Ai introdus valoarea boolean: " + booleanIntrodus);

        System.out.println("Introduceti caracterul dorit: ");
        char caracterIntrodus = cititor.next().charAt(0);
    }
}
